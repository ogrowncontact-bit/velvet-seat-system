...
-- Never expose the raw Stripe account id to anon/authenticated directly —
-- server functions read it with the service-role client. Booking UI only
-- needs to know whether the restaurant *can* take a deposit right now.
GRANT SELECT (stripe_charges_enabled) ON public.restaurants TO anon, authenticated;

-- ============ 2. Reservation deposit tracking ============
-- Reuses the existing `deposit_amount numeric(10,2) default 0` column (added
-- back in the original schema for exactly this purpose but never wired up)
-- to hold the amount actually charged. We only add the two columns that
-- don't already exist: a status enum and the Stripe PaymentIntent id.
DO $$ BEGIN
